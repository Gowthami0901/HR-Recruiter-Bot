...
def whatsapp_webhook(request: Request, mongo_client: MongoClient):
    mongo_client = MongoClient('mongodb://localhost:27017/')  # Replace with your MongoDB connection string
    db = mongo_client['chat_database']
    chats_collection = db['chats']
    ...
    bot_response = "Automated response based on the message"

    save_chat_history(phone_number, user_message, bot_response)
...
def save_chat_history(phone_number, user_message, bot_response):
    ...
    # Determine interest based on the user message
    interest_status = "Interested" if "interested" in user_message.lower() else "Not Interested"
    ...
    if chat:
        ...
        if current_date in chat['history']:
            chat['history'][current_date].append({
                "user": user_message, 
                "bot": bot_response,
                "interest_status": interest_status
            })
        else:
            chat['history'][current_date] = [{
                "user": user_message, 
                "bot": bot_response,
                "interest_status": interest_status
            }]
        
        # Update the chat document with the new history and interest status
        chats_collection.update_one(
            {"chat_id": chat_id},
            {"$set": {"history": chat['history'], "interested": interest_status}}
        )
    else:
